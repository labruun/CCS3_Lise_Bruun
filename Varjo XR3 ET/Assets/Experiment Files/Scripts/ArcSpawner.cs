...
public class SpawnInAnnularSector : MonoBehaviour
{
    ...
    {
        int attempts = 0;
        while (spawnedPositions.Count < numberOfObjectsToSpawn && attempts < numberOfObjectsToSpawn * 10)
        {
    ...
    bool IsPositionValid(Vector3 position, List<Vector3> spawnedPositions)
    {
        ...
        if (!IsObjectFullyVisibleFromCamera(position))
        {
            return false;
        }
