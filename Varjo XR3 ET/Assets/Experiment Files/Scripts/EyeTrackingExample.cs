...
public class EyeTrackingExample : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Physics.SphereCast(rayOrigin, gazeRadius, direction, out hit))
        {
            ...
            // Highlight gaze when looked at
            HighlightOnGaze highlightOnGaze = hit.collider.gameObject.GetComponent<HighlightOnGaze>();

            if (highlightOnGaze != null)
            {
                highlightOnGaze.RayHit();
            }
        }
        else
        {
